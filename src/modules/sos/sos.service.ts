...
export class SOSService {
    ...
    try {
      ...
      if (videoFile) {
        try {
          ...
          logger.error(
            "Failed to upload SOS video, proceeding without video:",
            error,
          );
  ...
  async updateStatus(
    ...
    SOSStateMachine.validateTransitionWithContext(sosCase.status, data.status, {
      hasOfficer: !!sosCase.officerId,
      isOfficer:
        userRole === UserRole.OFFICER && sosCase.officer?.userId === userId,
    ...
    const updated = await prisma.sosCase.update({
      ...
      data: {
        ...
        closureNotes:
          data.status === CaseStatus.CLOSED ? data.notes : undefined,
      },
      include: {
        citizen: {
          include: { user: true },
    ...
    logger.info(
      `Case ${sosCase.caseNumber} status updated: ${sosCase.status} -> ${data.status}`,
    );
  ...
  async assignOfficer(caseId: string, officerId: string, assignedBy: string) {
    ...
    logger.info(
      `Case ${sosCase.caseNumber} assigned to officer ${officer.badgeNumber}`,
    );
  ...
  async addMedia(
    ...
    if (mediaType === "video") {
      ...
      // Handle photos by creating Evidence record
      await prisma.evidence.create({
        data: {
          ...
          fileSize: file.size,
          uploadedBy: userId,
          // We could add lat/long here if available in the future
        },
      });
    }
